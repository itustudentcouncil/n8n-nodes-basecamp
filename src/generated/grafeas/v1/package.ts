...
import { Digest, License } from "./common.js";
...
export const PackageNote: MessageFns<PackageNote> = {
  encode(message: PackageNote, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (message.name !== "") {
    ...
    if (message.version !== undefined) {
      Version.encode(message.version, writer.uint32(114).fork()).join();
    }
    if (message.maintainer !== "") {
    ...
    if (message.license !== undefined) {
      License.encode(message.license, writer.uint32(146).fork()).join();
    }
    for (const v of message.digest) {
      Digest.encode(v!, writer.uint32(154).fork()).join();
    }
    return writer;
  },

  decode(input: BinaryReader | Uint8Array, length?: number): PackageNote {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
  ...
  create(base?: DeepPartial<PackageNote>): PackageNote {
    return PackageNote.fromPartial(base ?? {});
  },
  fromPartial(object: DeepPartial<PackageNote>): PackageNote {
    const message = createBasePackageNote();
...
export const PackageOccurrence: MessageFns<PackageOccurrence> = {
  encode(message: PackageOccurrence, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (message.name !== "") {
      writer.uint32(10).string(message.name);
    }
    ...
    if (message.license !== undefined) {
      License.encode(message.license, writer.uint32(50).fork()).join();
    }
    if (message.version !== undefined) {
      Version.encode(message.version, writer.uint32(58).fork()).join();
    }
    return writer;
  },

  decode(input: BinaryReader | Uint8Array, length?: number): PackageOccurrence {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
  ...
  create(base?: DeepPartial<PackageOccurrence>): PackageOccurrence {
    return PackageOccurrence.fromPartial(base ?? {});
  },
  fromPartial(object: DeepPartial<PackageOccurrence>): PackageOccurrence {
    const message = createBasePackageOccurrence();
    message.name = object.name ?? "";
