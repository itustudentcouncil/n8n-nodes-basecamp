...
export const CVSS: MessageFns<CVSS> = {
  encode(message: CVSS, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (message.baseScore !== 0) {
      writer.uint32(13).float(message.baseScore);
    }
    if (message.exploitabilityScore !== 0) {
  ...
  decode(input: BinaryReader | Uint8Array, length?: number): CVSS {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
  ...
  create(base?: DeepPartial<CVSS>): CVSS {
    return CVSS.fromPartial(base ?? {});
  },
  fromPartial(object: DeepPartial<CVSS>): CVSS {
    const message = createBaseCVSS();
    message.baseScore = object.baseScore ?? 0;
    message.exploitabilityScore = object.exploitabilityScore ?? 0;
    message.impactScore = object.impactScore ?? 0;
