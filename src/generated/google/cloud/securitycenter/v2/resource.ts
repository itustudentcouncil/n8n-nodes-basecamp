...
function createBaseGcpMetadata(): GcpMetadata {
  return { project: "", projectDisplayName: "", parent: "", parentDisplayName: "", folders: [], organization: "" };
}

export const GcpMetadata: MessageFns<GcpMetadata> = {
  encode(message: GcpMetadata, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (message.project !== "") {
      writer.uint32(10).string(message.project);
    }
    if (message.projectDisplayName !== "") {
      writer.uint32(18).string(message.projectDisplayName);
    }
    if (message.parent !== "") {
      writer.uint32(26).string(message.parent);
    }
    if (message.parentDisplayName !== "") {
      writer.uint32(34).string(message.parentDisplayName);
    }
    for (const v of message.folders) {
      Folder.encode(v!, writer.uint32(42).fork()).join();
    }
  ...
  decode(input: BinaryReader | Uint8Array, length?: number): GcpMetadata {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = createBaseGcpMetadata();
    while (reader.pos < end) {
      const tag = reader.uint32();
      switch (tag >>> 3) {
        case 1:
  ...
  fromJSON(object: any): GcpMetadata {
    return {
  ...
  toJSON(message: GcpMetadata): unknown {
    const obj: any = {};
    if (message.project !== "") {
  ...
  create(base?: DeepPartial<GcpMetadata>): GcpMetadata {
    return GcpMetadata.fromPartial(base ?? {});
  },
  fromPartial(object: DeepPartial<GcpMetadata>): GcpMetadata {
    const message = createBaseGcpMetadata();
...
export const AwsMetadata: MessageFns<AwsMetadata> = {
  encode(message: AwsMetadata, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (message.organization !== undefined) {
      AwsMetadata_AwsOrganization.encode(message.organization, writer.uint32(10).fork()).join();
    }
    for (const v of message.organizationalUnits) {
      AwsMetadata_AwsOrganizationalUnit.encode(v!, writer.uint32(18).fork()).join();
    }
    if (message.account !== undefined) {
      AwsMetadata_AwsAccount.encode(message.account, writer.uint32(26).fork()).join();
    }
    return writer;
  },
...
export const AzureMetadata: MessageFns<AzureMetadata> = {
  encode(message: AzureMetadata, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    for (const v of message.managementGroups) {
      AzureMetadata_AzureManagementGroup.encode(v!, writer.uint32(10).fork()).join();
    }
    if (message.subscription !== undefined) {
      AzureMetadata_AzureSubscription.encode(message.subscription, writer.uint32(18).fork()).join();
    }
    if (message.resourceGroup !== undefined) {
      AzureMetadata_AzureResourceGroup.encode(message.resourceGroup, writer.uint32(26).fork()).join();
    }
    return writer;
  },
