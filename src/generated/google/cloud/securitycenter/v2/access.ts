...
export const Access: MessageFns<Access> = {
  encode(message: Access, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    ...
    if (message.methodName !== "") {
      writer.uint32(58).string(message.methodName);
    }
    if (message.principalSubject !== "") {
      writer.uint32(66).string(message.principalSubject);
    }
    if (message.serviceAccountKeyName !== "") {
      writer.uint32(74).string(message.serviceAccountKeyName);
    }
    for (const v of message.serviceAccountDelegationInfo) {
      ServiceAccountDelegationInfo.encode(v!, writer.uint32(82).fork()).join();
    }
    if (message.userName !== "") {
