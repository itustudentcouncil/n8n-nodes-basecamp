...
function createBaseAppEngineHttpQueue(): AppEngineHttpQueue {
  return { appEngineRoutingOverride: undefined };
}

export const AppEngineHttpQueue: MessageFns<AppEngineHttpQueue> = {
  encode(message: AppEngineHttpQueue, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
  ...
  decode(input: BinaryReader | Uint8Array, length?: number): AppEngineHttpQueue {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = createBaseAppEngineHttpQueue();
    while (reader.pos < end) {
      const tag = reader.uint32();
      switch (tag >>> 3) {
        case 1:
  ...
  fromJSON(object: any): AppEngineHttpQueue {
    return {
  ...
  toJSON(message: AppEngineHttpQueue): unknown {
    const obj: any = {};
    if (message.appEngineRoutingOverride !== undefined) {
  ...
  create(base?: DeepPartial<AppEngineHttpQueue>): AppEngineHttpQueue {
    return AppEngineHttpQueue.fromPartial(base ?? {});
  },
  fromPartial(object: DeepPartial<AppEngineHttpQueue>): AppEngineHttpQueue {
    const message = createBaseAppEngineHttpQueue();
