...
function createBaseVolume(): Volume {
  return { nfs: undefined, pd: undefined, gcs: undefined, deviceName: undefined, mountPath: "", mountOptions: [] };
}
...
export const Volume: MessageFns<Volume> = {
  encode(message: Volume, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    ...
    if (message.pd !== undefined) {
      PD.encode(message.pd, writer.uint32(18).fork()).join();
    }
    ...
    while (reader.pos < end) {
      ...
      switch (tag >>> 3) {
        ...
        case 2:
          ...
          message.pd = PD.decode(reader, reader.uint32());
          continue;
        case 3:
          if (tag !== 26) {
            break;
  ...
  fromJSON(object: any): Volume {
    return {
      ...
      pd: isSet(object.pd) ? PD.fromJSON(object.pd) : undefined,
  ...
  fromPartial(object: DeepPartial<Volume>): Volume {
    ...
    message.pd = (object.pd !== undefined && object.pd !== null) ? PD.fromPartial(object.pd) : undefined;
...
function createBasePD(): PD {
  return { disk: "", device: "", existing: false };
}

export const PD: MessageFns<PD> = {
  encode(message: PD, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (message.disk !== "") {
  ...
  decode(input: BinaryReader | Uint8Array, length?: number): PD {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = createBasePD();
    while (reader.pos < end) {
      const tag = reader.uint32();
      switch (tag >>> 3) {
        case 1:
  ...
  create(base?: DeepPartial<PD>): PD {
    return PD.fromPartial(base ?? {});
  },
  fromPartial(object: DeepPartial<PD>): PD {
    const message = createBasePD();
