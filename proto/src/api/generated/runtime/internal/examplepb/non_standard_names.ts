...
export const NonStandardMessage: MessageFns<NonStandardMessage> = {
  encode(message: NonStandardMessage, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    ...
    if (message.structField !== undefined) {
      Struct.encode(Struct.wrap(message.structField), writer.uint32(74).fork()).join();
    }
    if (message.valueField !== undefined) {
      Value.encode(Value.wrap(message.valueField), writer.uint32(82).fork()).join();
    }
    return writer;
  },
...
export const NonStandardServiceDefinition = {
  ...
  fullName: "grpc.gateway.runtime.internal.examplepb.NonStandardService",
  methods: {
