...
export const File: MessageFns<File> = {
  encode(message: File, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    ...
    if (message.fetchTime !== undefined) {
      Timestamp.encode(toTimestamp(message.fetchTime), writer.uint32(66).fork()).join();
    }
    return writer;
  },
    ...
    while (reader.pos < end) {
      ...
      switch (tag >>> 3) {
        ...
        case 8:
          ...
          message.fetchTime = fromTimestamp(Timestamp.decode(reader, reader.uint32()));
          continue;
      }
      if ((tag & 7) === 4 || tag === 0) {
        break;
  ...
  toJSON(message: File): unknown {
    ...
    if (message.fetchTime !== undefined) {
      obj.fetchTime = message.fetchTime.toISOString();
    }
    return obj;
  },
  ...
  fromPartial(object: DeepPartial<File>): File {
    ...
    message.fetchTime = object.fetchTime ?? undefined;
    return message;
  },
};
