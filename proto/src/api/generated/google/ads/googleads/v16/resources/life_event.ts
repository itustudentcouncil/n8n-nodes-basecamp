...
function createBaseLifeEvent(): LifeEvent {
  return { resourceName: "", id: Long.ZERO, name: "", parent: "", launchedToAll: false, availabilities: [] };
}

export const LifeEvent: MessageFns<LifeEvent> = {
  encode(message: LifeEvent, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
  ...
  decode(input: BinaryReader | Uint8Array, length?: number): LifeEvent {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = createBaseLifeEvent();
  ...
  toJSON(message: LifeEvent): unknown {
    const obj: any = {};
    if (message.resourceName !== "") {
  ...
  create(base?: DeepPartial<LifeEvent>): LifeEvent {
    return LifeEvent.fromPartial(base ?? {});
  },
  fromPartial(object: DeepPartial<LifeEvent>): LifeEvent {
    const message = createBaseLifeEvent();
