...
export const Task: MessageFns<Task> = {
  encode(message: Task, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    ...
    if (message.httpRequest !== undefined) {
      HttpRequest.encode(message.httpRequest, writer.uint32(90).fork()).join();
    }
    if (message.pullMessage !== undefined) {
      PullMessage.encode(message.pullMessage, writer.uint32(106).fork()).join();
    }
    ...
    if (message.dispatchDeadline !== undefined) {
      Duration.encode(message.dispatchDeadline, writer.uint32(98).fork()).join();
    }
    if (message.dispatchCount !== 0) {
      writer.uint32(48).int32(message.dispatchCount);
    }
    if (message.responseCount !== 0) {
      writer.uint32(56).int32(message.responseCount);
    }
    if (message.firstAttempt !== undefined) {
      Attempt.encode(message.firstAttempt, writer.uint32(66).fork()).join();
    }
    if (message.lastAttempt !== undefined) {
      Attempt.encode(message.lastAttempt, writer.uint32(74).fork()).join();
    }
    if (message.view !== 0) {
      writer.uint32(80).int32(message.view);
    }
    return writer;
  },
