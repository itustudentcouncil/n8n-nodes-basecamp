...
export interface MarkRecommendationDismissedRequest {
  /** Required. Name of the recommendation. */
  name: string;
...
function createBaseMarkRecommendationDismissedRequest(): MarkRecommendationDismissedRequest {
  return { name: "", etag: "" };
}

export const MarkRecommendationDismissedRequest: MessageFns<MarkRecommendationDismissedRequest> = {
  encode(message: MarkRecommendationDismissedRequest, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (message.name !== "") {
      writer.uint32(10).string(message.name);
    }
    if (message.etag !== "") {
  ...
  decode(input: BinaryReader | Uint8Array, length?: number): MarkRecommendationDismissedRequest {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = createBaseMarkRecommendationDismissedRequest();
    while (reader.pos < end) {
      const tag = reader.uint32();
      switch (tag >>> 3) {
        case 1:
  ...
  fromJSON(object: any): MarkRecommendationDismissedRequest {
    return {
      name: isSet(object.name) ? globalThis.String(object.name) : "",
      etag: isSet(object.etag) ? globalThis.String(object.etag) : "",
    };
  ...
  toJSON(message: MarkRecommendationDismissedRequest): unknown {
    const obj: any = {};
    if (message.name !== "") {
      obj.name = message.name;
    }
  ...
  create(base?: DeepPartial<MarkRecommendationDismissedRequest>): MarkRecommendationDismissedRequest {
    return MarkRecommendationDismissedRequest.fromPartial(base ?? {});
  },
  fromPartial(object: DeepPartial<MarkRecommendationDismissedRequest>): MarkRecommendationDismissedRequest {
    const message = createBaseMarkRecommendationDismissedRequest();
    message.name = object.name ?? "";
    message.etag = object.etag ?? "";
    return message;
...
export const RecommenderDefinition = {
  ...
  methods: {
      ...
      options: {
        _unknownFields: {
          8410: [
            ...
            Buffer.from([13, 112, 97, 114, 101, 110, 116, 44, 102, 105, 108, 116, 101, 114]),
          ],
          578365826: [
            Buffer.from([
              161,
    ...
    markRecommendationDismissed: {
      ...
      requestType: MarkRecommendationDismissedRequest,
      requestStream: false,
      responseType: Recommendation,
      responseStream: false,
      options: {
...
export interface RecommenderClient<CallOptionsExt = {}> {
   ...
   */
  markRecommendationDismissed(
    request: DeepPartial<MarkRecommendationDismissedRequest>,
    options?: CallOptions & CallOptionsExt,
  ): Promise<Recommendation>;
  /**
   * Marks the Recommendation State as Claimed. Users can use this method to
