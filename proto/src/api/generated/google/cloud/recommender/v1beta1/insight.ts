...
function createBaseInsightType(): InsightType {
  return { name: "" };
}

export const InsightType: MessageFns<InsightType> = {
  encode(message: InsightType, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (message.name !== "") {
      writer.uint32(10).string(message.name);
    }
    return writer;
  ...
  decode(input: BinaryReader | Uint8Array, length?: number): InsightType {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = createBaseInsightType();
    while (reader.pos < end) {
      const tag = reader.uint32();
      switch (tag >>> 3) {
        case 1:
  ...
  fromJSON(object: any): InsightType {
    return { name: isSet(object.name) ? globalThis.String(object.name) : "" };
  },

  toJSON(message: InsightType): unknown {
    const obj: any = {};
    if (message.name !== "") {
      obj.name = message.name;
    }
  ...
  create(base?: DeepPartial<InsightType>): InsightType {
    return InsightType.fromPartial(base ?? {});
  },
  fromPartial(object: DeepPartial<InsightType>): InsightType {
    const message = createBaseInsightType();
    message.name = object.name ?? "";
    return message;
  },
