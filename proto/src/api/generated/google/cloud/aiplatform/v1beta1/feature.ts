...
export const Feature: MessageFns<Feature> = {
  encode(message: Feature, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    ...
    if (message.monitoringConfig !== undefined) {
      FeaturestoreMonitoringConfig.encode(message.monitoringConfig, writer.uint32(74).fork()).join();
    }
    if (message.disableMonitoring !== false) {
      writer.uint32(96).bool(message.disableMonitoring);
    }
    for (const v of message.monitoringStats) {
      FeatureStatsAnomaly.encode(v!, writer.uint32(82).fork()).join();
    }
    ...
    while (reader.pos < end) {
      ...
      switch (tag >>> 3) {
        ...
        case 10:
          ...
          message.monitoringStats.push(FeatureStatsAnomaly.decode(reader, reader.uint32()));
          continue;
        case 11:
          if (tag !== 90) {
            break;
  ...
  fromJSON(object: any): Feature {
    return {
      ...
      monitoringStats: globalThis.Array.isArray(object?.monitoringStats)
        ? object.monitoringStats.map((e: any) => FeatureStatsAnomaly.fromJSON(e))
        : [],
  ...
  toJSON(message: Feature): unknown {
    ...
    if (message.monitoringStats?.length) {
      obj.monitoringStats = message.monitoringStats.map((e) => FeatureStatsAnomaly.toJSON(e));
    }
  ...
  fromPartial(object: DeepPartial<Feature>): Feature {
    ...
    message.monitoringStats = object.monitoringStats?.map((e) => FeatureStatsAnomaly.fromPartial(e)) || [];
