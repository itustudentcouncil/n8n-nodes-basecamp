...
function createBaseCreateMessageRequest(): CreateMessageRequest {
  return { parent: "", message: undefined };
}
...
function createBaseBatchCreateMessagesRequest(): BatchCreateMessagesRequest {
  return { parent: "", requests: [] };
}

export const BatchCreateMessagesRequest: MessageFns<BatchCreateMessagesRequest> = {
  encode(message: BatchCreateMessagesRequest, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (message.parent !== "") {
      writer.uint32(10).string(message.parent);
    }
    for (const v of message.requests) {
      CreateMessageRequest.encode(v!, writer.uint32(18).fork()).join();
    }
    return writer;
  },

  decode(input: BinaryReader | Uint8Array, length?: number): BatchCreateMessagesRequest {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = createBaseBatchCreateMessagesRequest();
    while (reader.pos < end) {
      const tag = reader.uint32();
      switch (tag >>> 3) {
        case 1:
        ...
        case 2:
          ...
          message.requests.push(CreateMessageRequest.decode(reader, reader.uint32()));
          continue;
      }
      if ((tag & 7) === 4 || tag === 0) {
        break;
  ...
  fromJSON(object: any): BatchCreateMessagesRequest {
    return {
      parent: isSet(object.parent) ? globalThis.String(object.parent) : "",
      requests: globalThis.Array.isArray(object?.requests)
        ? object.requests.map((e: any) => CreateMessageRequest.fromJSON(e))
        : [],
    };
  },

  toJSON(message: BatchCreateMessagesRequest): unknown {
    const obj: any = {};
    if (message.parent !== "") {
      obj.parent = message.parent;
    }
    if (message.requests?.length) {
      obj.requests = message.requests.map((e) => CreateMessageRequest.toJSON(e));
    }
    return obj;
  },

  create(base?: DeepPartial<BatchCreateMessagesRequest>): BatchCreateMessagesRequest {
    return BatchCreateMessagesRequest.fromPartial(base ?? {});
  },
  fromPartial(object: DeepPartial<BatchCreateMessagesRequest>): BatchCreateMessagesRequest {
    const message = createBaseBatchCreateMessagesRequest();
    message.parent = object.parent ?? "";
    message.requests = object.requests?.map((e) => CreateMessageRequest.fromPartial(e)) || [];
    return message;
  },
};

function createBaseBatchCreateMessagesResponse(): BatchCreateMessagesResponse {
  return { messages: [] };
}

export const BatchCreateMessagesResponse: MessageFns<BatchCreateMessagesResponse> = {
  encode(message: BatchCreateMessagesResponse, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
  ...
  decode(input: BinaryReader | Uint8Array, length?: number): BatchCreateMessagesResponse {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = createBaseBatchCreateMessagesResponse();
    while (reader.pos < end) {
      const tag = reader.uint32();
      switch (tag >>> 3) {
        case 1:
  ...
  fromJSON(object: any): BatchCreateMessagesResponse {
    return {
      messages: globalThis.Array.isArray(object?.messages) ? object.messages.map((e: any) => Message.fromJSON(e)) : [],
    };
  },

  toJSON(message: BatchCreateMessagesResponse): unknown {
    const obj: any = {};
  ...
  create(base?: DeepPartial<BatchCreateMessagesResponse>): BatchCreateMessagesResponse {
    return BatchCreateMessagesResponse.fromPartial(base ?? {});
  },
  fromPartial(object: DeepPartial<BatchCreateMessagesResponse>): BatchCreateMessagesResponse {
    const message = createBaseBatchCreateMessagesResponse();
...
export interface ConversationsServiceImplementation<CallContextExt = {}> {
  ...
  batchCreateMessages(
    ...
    context: CallContext & CallContextExt,
  ): Promise<DeepPartial<BatchCreateMessagesResponse>>;
  /**
   * Lists messages that belong to a given conversation.
   * `messages` are ordered by `create_time` in descending order. To fetch
   * updates without duplication, send request with filter
...
export interface ConversationsClient<CallOptionsExt = {}> {
   ...
   */
  batchCreateMessages(
    request: DeepPartial<BatchCreateMessagesRequest>,
    options?: CallOptions & CallOptionsExt,
