...
function createBaseOSPolicyAssignment_InstanceFilter(): OSPolicyAssignment_InstanceFilter {
  return { all: false, osShortNames: [], inclusionLabels: [], exclusionLabels: [], inventories: [] };
}
...
export const OSPolicyAssignment_InstanceFilter: MessageFns<OSPolicyAssignment_InstanceFilter> = {
  encode(message: OSPolicyAssignment_InstanceFilter, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    ...
    for (const v of message.osShortNames) {
    ...
    for (const v of message.exclusionLabels) {
      OSPolicyAssignment_LabelSet.encode(v!, writer.uint32(34).fork()).join();
    }
    for (const v of message.inventories) {
      OSPolicyAssignment_InstanceFilter_Inventory.encode(v!, writer.uint32(42).fork()).join();
    }
    return writer;
  },
    ...
    while (reader.pos < end) {
      ...
      switch (tag >>> 3) {
        ...
        case 2:
          ...
          message.osShortNames.push(reader.string());
          continue;
        case 3:
          if (tag !== 26) {
            break;
  ...
  fromJSON(object: any): OSPolicyAssignment_InstanceFilter {
    return {
      ...
      osShortNames: globalThis.Array.isArray(object?.osShortNames)
        ? object.osShortNames.map((e: any) => globalThis.String(e))
        : [],
  ...
  toJSON(message: OSPolicyAssignment_InstanceFilter): unknown {
    ...
    if (message.osShortNames?.length) {
      obj.osShortNames = message.osShortNames;
    }
  ...
  fromPartial(object: DeepPartial<OSPolicyAssignment_InstanceFilter>): OSPolicyAssignment_InstanceFilter {
    ...
    message.osShortNames = object.osShortNames?.map((e) => e) || [];
