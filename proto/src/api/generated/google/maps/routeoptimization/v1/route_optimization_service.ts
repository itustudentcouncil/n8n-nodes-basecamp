...
export const protobufPackage = "google.maps.routeoptimization.v1";
...
export enum DataFormat {
  ...
  JSON = 1,
  /**
   * PROTO_TEXT - Protocol Buffers text format.  See
...
export interface BatchOptimizeToursMetadata {
}
...
export enum OptimizeToursRequest_SolvingMode {
  /**
   * DEFAULT_SOLVE - Solve the model. Warnings may be issued in
...
export interface OptimizeToursResponse {
  ...
  metrics: OptimizeToursResponse_Metrics | undefined;
}
...
function createBaseBatchOptimizeToursRequest_AsyncModelConfig(): BatchOptimizeToursRequest_AsyncModelConfig {
  return { displayName: "", inputConfig: undefined, outputConfig: undefined };
}
...
function createBaseBatchOptimizeToursMetadata(): BatchOptimizeToursMetadata {
  return {};
}

export const BatchOptimizeToursMetadata: MessageFns<BatchOptimizeToursMetadata> = {
  encode(_: BatchOptimizeToursMetadata, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    return writer;
  },

  decode(input: BinaryReader | Uint8Array, length?: number): BatchOptimizeToursMetadata {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = createBaseBatchOptimizeToursMetadata();
    while (reader.pos < end) {
      const tag = reader.uint32();
      switch (tag >>> 3) {
      }
  ...
  fromJSON(_: any): BatchOptimizeToursMetadata {
    return {};
  },

  toJSON(_: BatchOptimizeToursMetadata): unknown {
    const obj: any = {};
    return obj;
  },

  create(base?: DeepPartial<BatchOptimizeToursMetadata>): BatchOptimizeToursMetadata {
    return BatchOptimizeToursMetadata.fromPartial(base ?? {});
  },
  fromPartial(_: DeepPartial<BatchOptimizeToursMetadata>): BatchOptimizeToursMetadata {
    const message = createBaseBatchOptimizeToursMetadata();
    return message;
  },
};
...
function createBaseOptimizeToursResponse(): OptimizeToursResponse {
  return { routes: [], requestLabel: "", skippedShipments: [], validationErrors: [], metrics: undefined };
}
...
export const Vehicle: MessageFns<Vehicle> = {
  encode(message: Vehicle, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (message.displayName !== "") {
      writer.uint32(258).string(message.displayName);
    }
 ...
 */
export type RouteOptimizationDefinition = typeof RouteOptimizationDefinition;
export const RouteOptimizationDefinition = {
...
export interface RouteOptimizationServiceImplementation<CallContextExt = {}> {
  /**
   * Sends an `OptimizeToursRequest` containing a `ShipmentModel` and returns an
   * `OptimizeToursResponse` containing `ShipmentRoute`s, which are a set of
   * routes to be performed by vehicles minimizing the overall cost.
...
export interface RouteOptimizationClient<CallOptionsExt = {}> {
  /**
   * Sends an `OptimizeToursRequest` containing a `ShipmentModel` and returns an
   * `OptimizeToursResponse` containing `ShipmentRoute`s, which are a set of
   * routes to be performed by vehicles minimizing the overall cost.
