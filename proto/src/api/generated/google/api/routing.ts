...
export const RoutingRule: MessageFns<RoutingRule> = {
  encode(message: RoutingRule, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    for (const v of message.routingParameters) {
      RoutingParameter.encode(v!, writer.uint32(18).fork()).join();
    }
    return writer;
  },
  ...
  fromJSON(object: any): RoutingRule {
    return {
      routingParameters: globalThis.Array.isArray(object?.routingParameters)
  ...
  toJSON(message: RoutingRule): unknown {
    const obj: any = {};
    if (message.routingParameters?.length) {
      obj.routingParameters = message.routingParameters.map((e) => RoutingParameter.toJSON(e));
    }
    return obj;
  },
...
export const RoutingParameter: MessageFns<RoutingParameter> = {
  encode(message: RoutingParameter, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (message.field !== "") {
