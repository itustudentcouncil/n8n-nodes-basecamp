...
export const AuthRequest: MessageFns<AuthRequest> = {
  ...
  fromJSON(object: any): AuthRequest {
    return {
      ...
      prompt: globalThis.Array.isArray(object?.prompt) ? object.prompt.map((e: any) => promptFromJSON(e)) : [],
      uiLocales: globalThis.Array.isArray(object?.uiLocales)
        ? object.uiLocales.map((e: any) => globalThis.String(e))
        : [],
...
export const AuthorizationError: MessageFns<AuthorizationError> = {
  encode(message: AuthorizationError, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (message.error !== 0) {
  ...
  decode(input: BinaryReader | Uint8Array, length?: number): AuthorizationError {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
