...
export const Editor: MessageFns<Editor> = {
  encode(message: Editor, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (message.userId !== "") {
  ...
  decode(input: BinaryReader | Uint8Array, length?: number): Editor {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
...
export const AggregateType: MessageFns<AggregateType> = {
  encode(message: AggregateType, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (message.type !== "") {
      writer.uint32(10).string(message.type);
    }
    if (message.localized !== undefined) {
  ...
  decode(input: BinaryReader | Uint8Array, length?: number): AggregateType {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
