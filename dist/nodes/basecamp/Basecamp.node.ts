...
const actionToLabel: { [key in keyof (typeof routes)["people"]]: string } = {
  index: "Get all",
  show: "Get",
  create: "Create",
  update: "Update",
...
const actions = Object.keys(routes).map((action) =>
  createAction(
    ...
    Object.keys(routes[action]).map((key) => ({
      name: actionToLabel[key as keyof typeof actionToLabel],
      value: key
    }))
  )
)
...
export class Basecamp implements INodeType {
  ...
  async execute(this: IExecuteFunctions) {
    ...
    let body: string | number | boolean | object
